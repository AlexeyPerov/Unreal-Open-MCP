...
- `cli/` — `unreal-open-mcp-cli` setup/ops CLI (`cli/src/index.ts` entry, `cli/src/cli.ts` dispatcher, `cli/src/args.ts` argv parser, `cli/src/help-text.ts` help/version formatters, `cli/src/commands/install-plugin.ts` + `cli/src/commands/setup-mcp.ts` + `cli/src/commands/open.ts` + `cli/src/commands/wait-for-ready.ts` + `cli/src/commands/status.ts` + `cli/src/commands/configure.ts` implemented commands, `cli/src/lib/install-plugin.ts` + `cli/src/lib/uproject.ts` + `cli/src/lib/plugin-source.ts` install internals, `cli/src/lib/setup-mcp.ts` + `cli/src/lib/agents.ts` stdio MCP-client config writer, `cli/src/lib/open.ts` engine resolve + detached editor spawn, `cli/src/lib/port-discovery.ts` thin CLI-side mirror of the per-project port formula + instance-lock read, `cli/src/lib/ping-poller.ts` compile-aware `/ping` readiness poller, `cli/src/lib/status.ts` pure bridge-status mapper + report composition (mirrors the MCP `unreal_open_mcp_bridge_status` tool's status tokens), `cli/src/lib/configure.ts` read/write of `<project>/.unreal-open-mcp/settings.json` — the same project-local settings file the bridge reads `authMode` from). Separate publishable package from the stdio MCP server (ADR-007); the CLI mirrors the mcp-server's port-discovery + bridge-status contract locally (same deterministic hash + lock precedence + status-mapping table) rather than importing the mcp-server package, so it has no runtime deps.
- `skills/` — agent playbooks (`SKILL.md`).
- `demo/` — minimal Unreal C++ demo project with fixtures.
- `validation-suite/` — standalone Tauri + SvelteKit guided manual-validation app (targets `demo/` via the `unreal` engine profile; engine-neutral orchestration in `packages/core/`).
