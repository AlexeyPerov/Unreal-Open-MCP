...
// material_modify / material_get_data). Further tools land in later phases and
// append here.
test("handleListTools returns the registered tools", async () => {
  const result = await handleListTools();
  assert.equal(result.tools.length, 49);
...
test("handleCallTool resolves unreal_open_mcp_capabilities locally without a router", async () => {
  ...
  assert.equal(payload.counts.toolsImplemented, 49);
