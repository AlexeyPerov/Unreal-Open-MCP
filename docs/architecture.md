...
The source family (`source_read`, `source_list`) is the read spine for project C++ under `<Project>/Source/`. It is distinct from the asset family (`/Game` content via the AssetRegistry) and from the plugin's own source under `Plugins/UnrealOpenMCP/Source/` — the jail is the loaded project's `Source/` only. The jail contract (`ResolveJailedPath`) is exported from `UnrealOpenMcpSourceTools` so the Automation specs drive it directly with an injectable temp root (no live editor required); later mutating source tools (create / update / delete / compile) reuse the same jail so a path that escapes is rejected before any file op runs.
