...
- Module layout:
  ...
  - `Private/Core/`, `Private/Fixes/` — implementations.
...
- **Every `FVerifyIssue` must carry an `IssueCode`**. v1 issue codes (sketch): `broken_soft_reference`, `missing_blueprint_parent`, `compile_error`, `content_path_hygiene`. Issue codes link rules to fixes.
