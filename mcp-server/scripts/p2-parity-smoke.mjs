#!/usr/bin/env node
// p2-parity-smoke.mjs — Phase 2 exit-gate parity smoke.
//
// Canonical route: stdio MCP  →  unreal_open_mcp_actor_find  →  bridge
// POST /tools/unreal_open_mcp_actor_find  →  {ok,result,error} envelope.
//
// This is the MANDATORY gate before Phase 3 (gate/verify) begins (roadmap
// Phase 2 exit gate). P1.9 proved the /ping route end-to-end; Phase 2 widened
// the dispatch to every other tool via POST /tools/{name} (P2.1) and shipped
// the first typed tool, unreal_open_mcp_actor_find (P2.2). This smoke proves
// the FULL typed-tool round-trip survives the built dist/index.js artifact over
// stdio — packaging, transport, and instance-discovery wiring the in-process
// integration suite (integration.test.ts P2.8 cases) cannot see.
//
// Three cases are pinned — the three the P2.8 acceptance criteria call out as
// load-bearing:
//   1. HEALTHY — stub serves GET /ping + POST /tools/unreal_open_mcp_actor_find
//      with {ok:true,result:<body>}; assert tools/list advertises actor_find,
...
//      surfaces bridge_offline (the typed-tool path inherits P1's failure
//      classification).
...
//      agent can branch on actor_not_found vs a transport error.
//
...
// fidelity, P2.8). Intentional delta: the stub dispatches by method + URL and
// reads the POST body so the {ok,result,error} envelope round-trip is exercised
// — P1's stub only answered /ping.
...
// Canonical actor-find targeted-hit body the bridge emits
// (FUnrealOpenMcpActorTools::HandleActorFind → ToActorData with
// bIncludeComponents=true). The stub wraps this in {ok:true,result:<body>}; the
// MCP text block carries the INNER object after LiveClient.postTool unwraps it.
// This is the parity pin on the actor-find field set.
  ...
  actors: [
    {
      ...
      components: [
        { name: "Sprite", class: "/Script/Engine.BillboardComponent" },
      ],
    },
  ],
  notFound: false,
...
// structured failure (e.g. the referenced actor does not resolve). The
// {ok:false,error:{code,message}} body surfaces as an MCP error carrying the
// tool-specific code so an agent can branch on the cause.
const ACTOR_FIND_MISS_ERROR = {
  ok: false,
  error: {
...
function printHelp() {
  console.error(`Usage: node scripts/p2-parity-smoke.mjs [options]
...
/** Handler that serves GET /ping (healthy) + POST actor_find → ok:true. */
async function healthyHandler(req, res) {
  ...
  if (
    req.method === "POST" &&
    req.url === "/tools/unreal_open_mcp_actor_find"
  ) {
    await readBody(req);
    res.writeHead(200, { "Content-Type": "application/json" });
...
/** Handler that serves POST actor_find → ok:false error envelope. */
async function toolErrorHandler(req, res) {
  if (
    ...
    await readBody(req);
    res.writeHead(200, { "Content-Type": "application/json" });
    res.end(JSON.stringify(ACTOR_FIND_MISS_ERROR));
    return;
  }
  if (req.method === "GET" && req.url === "/ping") {
    res.writeHead(200, { "Content-Type": "application/json" });
...
/**
 * Spawn the built server and drive initialize → tools/list → tools/call(actor_find).
  ...
  return new Promise((resolveResult, rejectResult) => {
    ...
    send({
      ...
      params: {
        ...
        capabilities: {},
        clientInfo: { name: "p2-smoke", version: "0.0.0" },
      },
    });
...
function assertCommonInit(outcome) {
  ...
  // smoke (ping for transport parity, actor_find as the typed-tool under
  ...
  check(
    "tools/list advertises unreal_open_mcp_actor_find",
    tools.includes("unreal_open_mcp_actor_find"),
    `got [${tools.join(", ")}]`,
  );
...
async function caseHealthy(opts) {
  console.log("\nCase 1: healthy actor_find round-trip (stub → ok:true)");
  ...
  try {
    ...
    check(
      "tools/call actor_find returns isError:false",
    ...
    check(
      "actor_find body is valid JSON",
      body !== undefined,
      "first content block was not a JSON text block",
    );
    if (body !== undefined) {
      // The INNER result object (not the {ok,result} envelope) must survive
      // the round-trip verbatim — the parity pin on the actor-find field set.
      check(
        "actor_find body matches the stub result exactly (envelope unwrapped)",
        JSON.stringify(body) === JSON.stringify(ACTOR_FIND_HIT),
...
async function caseBridgeDown(opts) {
  console.log("\nCase 2: bridge-down actor_find surfaces bridge_offline (dead port)");
  // Pin to port 1 — nothing listening, ECONNREFUSED. The server starts fine
  // (no preflight ping); tools/list is local; only tools/call(actor_find)
  ...
  const call = findById(messages, 3);
  check(
    "tools/call actor_find returns isError:true",
...
async function caseToolError(opts) {
  console.log("\nCase 3: tool-error actor_find surfaces the ok:false envelope (stub → ok:false)");
  ...
  try {
    ...
    if (body !== undefined) {
      ...
      check(
        "tool-error code is actor_not_found (tool-specific, not transport)",
        body?.error?.code === "actor_not_found",
        `code=${JSON.stringify(body?.error?.code)}`,
      );
      check(
        "tool-error message rides through verbatim",
        body?.error?.message === ACTOR_FIND_MISS_ERROR.error.message,
...
async function main() {
  ...
  console.log("unreal-open-mcp — Phase 2 parity smoke (actor_find)");
  ...
  // only — the body won't match ACTOR_FIND_HIT) and skip the other two.
  if (opts.port !== null) {
 ...
 * actor-find shape (actors array + notFound flag).
 */
async function caseHealthyLive(opts) {
  console.log("\nCase 1 (live): healthy actor_find round-trip (live bridge)");
  ...
  if (body !== undefined) {
    check(
      "actor_find body carries the actors[] + notFound shape",
      Array.isArray(body?.actors) && typeof body?.notFound === "boolean",
...
main().catch((err) => {
  console.error("p2-parity-smoke fatal:", err);
  process.exit(1);
});
